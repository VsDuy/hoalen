window.onload = function () {
	...
	// Thêm chọn mức giá
	addPricesRange(0, 10000);
	addPricesRange(10000, 50000);
	addPricesRange(50000, 200000);
	addPricesRange(200000, 500000);
	addPricesRange(500000, 0);
...
function priceToString(min, max) {
	if (min == 0) return 'Dưới ' + max + ' đ';
	if (max == 0) return 'Trên ' + min + ' đ';
	return 'Từ ' + min + ' - ' + max + ' đ';
}
